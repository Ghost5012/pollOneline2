...
def trainer(request,id):
    ...
    def getImagesWithID(path):
        ...
        for imagePath in imagePaths:
            ...
            ID = int(os.path.split(imagePath)[-1].split('.')[0]) # -1 so that it will count from backwards and slipt the second index of the '.' Hence id
            # Images
            faces.append(faceNp)
            # Label
            Ids.append(ID)
...
def create_dataset(request):
    ...
    cv2.destroyAllWindows()
    trainer(request,id)
    path=BASE_DIR+'/ml/dataset/'+str(id)+'.'+str(1)+'.jpg'
    request.session['userId']=userId
    return redirect(reverse('poll:ap'))#Erreur
